...
namespace BookingHotel.Common.Helpers
{
    public class JWTExtensions
    {
        ...
        public static int GetDecodeTokenById(string token)
        {
            ...
            var jsonToken = handler.ReadToken(authHeader);
            var tokenS = handler.ReadToken(authHeader) as JwtSecurityToken;
            return tokenS.Claims.First(x => x.Type.Equals("unique_name")).Value.ToInt();
        }
        public static int GetDecodeTokenByPartnerId(string token)
        {
            var handler = new JwtSecurityTokenHandler();
            string authHeader = token.ToSafetyString();
            authHeader = authHeader.Replace("Bearer ", "");
            var jsonToken = handler.ReadToken(authHeader);
            var tokenS = handler.ReadToken(authHeader) as JwtSecurityToken;
            return tokenS.Claims.First(x => x.Type.Equals("PartnerId")).Value.ToInt();
        }
